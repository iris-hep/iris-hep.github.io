layout: codas-hep-participant
e-mail: rhys.taus@cern.ch
institution: University of Rochester
name: Rhys Taus
photo: "/assets/images/codas-hep/2026/rhys-tays.png"
github-username: rhysics
linkedin-profile: https://www.linkedin.com/in/rhys-taus/
orcid:
title:
website: rhysics.github.io
logos:
    - /assets/images/codas-hep/logos/CMS.jpg
    - /assets/images/codas-hep/logos/university-of-rochester-logo-png.png
---

## My research:
Electroweak precision measurements, development of forward "trackless" electron reconstruction and radiation damage monitoring of the CMS HCAL endcaps.

## My expertise is:
Machine learning classification and data driven background estimation.

## A problem I'm grappling with:
Developing an interface class between different ntuple formats to preserve old analysis frameworks and new ntuple production.

## I've got my eyes on:
Combination of the weak mixing angle measurement across LHC experiments and testing the standard model to its limits in Run 3.

## I want to know more about:
Best practices for maintaining reproducable analysis frameworks.
